...
const BarChart = ({ isDashboard = false }) => {
  ...
  const normalizedData = chartData.map((entry) => ({
    key: entry.key,
    value: logScale((entry.value / maxValue) * 100).toFixed(2), // Adjust the scale as needed
  }));
  ...
  return (
    <div style={{ height: "600px" }}>
      <ResponsiveBar
        ...
        keys={["value"]}
        indexBy="key"
        margin={{ top: 30, right: 130, bottom: 50, left: 120 }}
        padding={0.3}
        colors={colors}
        tooltip={({ id, value, color }) => (
          <strong style={{ color }}>
            {id}: {value} (Original: {chartData.find(entry => entry.key === id)?.value} before conversion)
          </strong>
        )}
        ...
        axisLeft={{
          ...
          // tickPadding: 5,
          // tickRotation: 0,
          legend: "Memory usage (In exponents of 2)",
